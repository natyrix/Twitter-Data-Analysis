...
def preprocess_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    cols_2_drop = ['sentiment', 'possibly_sensitive']
    try:
...
def insert_to_tweet_table(dbName: str, df: pd.DataFrame, table_name: str) -> None:
    ...
    df = preprocess_df(df)
    df = df.loc[15926:len(df)]
        

    for _, row in df.iterrows():
        sqlQuery = f"""INSERT INTO {table_name} (created_at, source, full_text, polarity, subjectivity, language,
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('../data/processed_global_data_tweets.csv')
